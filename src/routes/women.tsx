...
export const Route = createFileRoute("/women")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Niqabs and heritage essentials for women." },
    ],
  }),
  component: WomenPage,
});
...
function WomenPage() {
  const { add } = useCart();
  const items = products.filter((p) => p.to === "/niqabs");
  ...
  return (
    <div className="min-h-screen bg-ivory text-ink">
      ...
      <section className="relative h-[44vh] min-h-[280px] overflow-hidden bg-cream">
        <img src={niqab} alt="Women's collection" className="absolute inset-0 h-full w-full object-cover" />
        ...
        <div className="relative z-10 h-full flex items-center justify-center text-center text-ivory">
          <div>
            ...
            <h1 className="font-display text-5xl md:text-7xl mt-3">Women</h1>
          </div>
        </div>
      </section>
