...
function NiqabPage() {
  return (
    <div className="min-h-screen bg-ink text-ivory pb-20 md:pb-0">
      ...
      <section className="relative min-h-[100svh] flex items-end overflow-hidden">
        ...
        <div className="relative w-full px-5 md:px-12 pb-14 md:pb-20 max-w-3xl">
          ...
          <div className="mt-7 flex flex-col sm:flex-row items-start sm:items-center gap-3 animate-fade-up" style={{ animationDelay: "0.3s" }}>
            <button className="inline-flex items-center justify-center gap-2 rounded-full bg-gold text-ink px-8 py-4 font-semibold hover:bg-gold-soft shadow-gold transition">
      ...
      {/* PRODUCT CARDS */}
      <section className="bg-gradient-to-b from-ink to-[oklch(0.12_0.008_60)] py-20 md:py-28">
        <div className="mx-auto max-w-6xl px-5 md:px-8">
          <h2 className="font-display text-3xl md:text-4xl text-center">Three silhouettes.</h2>
          <div className="mt-12 grid gap-5 md:grid-cols-3">
            {[
              { name: "Half Niqab", desc: "Single-layer, everyday wear", price: "$29" },
              { name: "Full Niqab", desc: "Two-layer chiffon, eye-line cut", price: "$39", badge: "Bestseller" },
              { name: "Niqab + Khimar Set", desc: "Matched set, head to waist", price: "$59" },
            ].map((p) => (
              <article key={p.name} className="relative rounded-sm bg-ink/60 border border-white/10 p-7 hover:border-gold transition group">
                ...
                <div className="mt-6 flex items-end justify-between">
                  ...
                  <button className="text-sm font-semibold underline underline-offset-4 decoration-gold/40 hover:decoration-gold">
                    Add to cart
                  </button>
                </div>
              </article>
