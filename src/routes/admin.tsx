...
const CATEGORIES = [
  { key: "shemaghs", label: "Shemaghs", blurb: "Men's heritage wraps and keffiyehs.", parent: "men", Icon: Shirt },
  { key: "kufis", label: "Kufis", blurb: "Men's prayer caps and daily essentials.", parent: "men", Icon: Package },
  { key: "niqabs", label: "Niqabs", blurb: "Women's modest face veils and coverings.", parent: "women", Icon: Heart },
  { key: "gloves", label: "Gloves", blurb: "Modest gloves and seasonal accessories.", parent: "unisex", Icon: Sparkles },
  { key: "honey", label: "Honey", blurb: "Pure honey and edible products.", parent: "unisex", Icon: PackageCheck },
];
...
function buildDashboardAnalytics(orders: AdminOrder[], customers: AdminCustomer[], daysCount = 7) {
  ...
  const revenueTrend = days.map((day) => {
    ...
    return {
      label: daysCount <= 7 ? day.toLocaleDateString("en-IN", { weekday: "short" }) : day.toLocaleDateString("en-IN", { day: "2-digit", month: "short" }),
...
const Admin = () => {
  ...
  const dashboard = useMemo(() => buildDashboardAnalytics(orders, customers, dashboardDays), [orders, customers, dashboardDays]);
  ...
  const paidOrders = orders.filter((o) => o.payment_status === "paid" || o.payment_status === "MOCKED_PAID");
  const revenueTotal = paidOrders.reduce((sum, order) => sum + (order.total_inr ?? order.total ?? 0), 0);
  const inTransitOrders = orders.filter((o) => normalizeOrderStatus(o.status) === "shipped" && Boolean(o.tracking_number)).length;
  const toActionOrders = processingOrders.length + shippedMissingTracking.length;
  const initials = (adminEmail || "HE")
    .split("@")[0]
    .split(/[._-]/)
    .filter(Boolean)
    .slice(0, 2)
  ...
  const navGroups: { label: string; items: typeof NAV[number][] }[] = [
    ...
    { label: "Commerce", items: NAV.filter((item) => ["orders", "products", "inventory", "categories", "shipping"].includes(item.key)) },
          ...
          <div className="mx-auto max-w-[1400px] space-y-6">
              ...
              {!loading && tab === "dash" && (
                <>
                  ...
                  <QuickAdminNav
                    items={[
                      { label: "Orders", value: orders.length, detail: `${toActionOrders} to action`, Icon: ShoppingBag, onClick: () => setTab("orders") },
                      { label: "Products", value: products.length, detail: `${opsStats.activeProducts} active`, Icon: Package, onClick: () => setTab("products") },
                      { label: "Inventory", value: opsStats.lowStock + opsStats.outOfStock, detail: "need attention", Icon: Boxes, onClick: () => setTab("inventory") },
                      { label: "Reviews", value: pendingReviews, detail: "pending", Icon: MessageSquare, onClick: () => setTab("reviews") },
                    ]}
                  />
                  ...
                  <ReviewsTable
                    ...
                    onStatusChange={async (id, status) => {
                      if (await updateReviewStatus(id, status)) {
                        notify({ title: status === "published" ? "Review published" : "Review updated" });
  ...
  return (
    <div className="vibe-admin admin-vibe flex min-h-screen bg-[rgb(var(--vibe-page))] text-[rgb(var(--vibe-foreground))]">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-30 flex h-14 items-center justify-between gap-3 border-b border-[#e5e5e5] bg-[#fafafa]/95 px-4 backdrop-blur md:px-6">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="hidden text-xs leading-tight text-[#737373] sm:block">Fawzaan store operations</p>
            </div>
          </div>
          <Link
            to="/"
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8">
          <div className="mx-auto max-w-[1400px] space-y-6">
            <Section title={authMode === "signIn" ? "Admin sign in" : "Create admin account"} subtitle="Use the store owner email configured in Convex.">
              <form onSubmit={onSubmit} className="max-w-md space-y-3">
                ...
                >
                  {authLoading || authSaving ? "Please wait..." : authMode === "signIn" ? "Sign in" : "Create account"}
                </button>
                <button
                  type="button"
                ...
                >
                  {authMode === "signIn" ? "First setup? Create admin account" : "Already created? Sign in"}
                </button>
              </form>
            </Section>
          </div>
...
function AttentionCard({
  ...
  return (
    <div className="vibe-card p-4 transition-colors hover:border-zinc-400 sm:p-5">
      ...
      <p className="line-clamp-2 text-[11px] text-[rgb(var(--vibe-muted))]">
        {description}
      </p>
    </div>
  );
        ...
        >
          ...
          <span className="min-w-0">
            ...
            <span className="mt-0.5 block text-xs text-[rgb(var(--vibe-muted))]">
              <span className="font-semibold tabular-nums text-[rgb(var(--vibe-foreground))]">{value}</span> {detail}
            </span>
          </span>
        </button>
      ))}
...
function TrendBadge({ value }: { value: number }) {
  return (
    <span className={cn("text-[11px] font-medium tabular-nums", value >= 0 ? "text-emerald-600" : "text-red-600")}>
      {value >= 0 ? "+" : ""}
      {value.toFixed(1)}%
    </span>
  );
    ...
    <div className="rounded-lg border border-border bg-background hover:border-foreground/20 transition-colors">
      ...
      <div className="space-y-3 p-4 md:hidden">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="font-mono text-[12px] leading-tight text-foreground/80" data-testid={`admin-mobile-order-number-${order.id}`}>
              {orderNumber}
            </p>
        ...
        <article key={p.id} className="rounded-lg border border-border bg-white p-3">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="line-clamp-2 text-[15px] font-semibold leading-snug text-[#111827]">{p.name}</p>
              ...
              <div className="mt-3 grid grid-cols-3 gap-2 text-sm">
                ...
                <div>
                  ...
                  <p className={cn("font-semibold", p.is_active === false ? "text-[#6B7280]" : "text-emerald-700")}>
          ...
          <article key={p.id} className="rounded-lg border border-[#E5E7EB] bg-white p-3">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-xs text-[#6B7280]">{p.sku || p.category || "No SKU"}</p>
              </div>
              <p className="shrink-0 text-lg font-semibold tabular-nums text-[#111827]">{stock}</p>
            </div>
...
function HomepageAdminPanel({
  ...
  return (
    <div className="space-y-6">
      ...
      <Section title="Homepage placement" subtitle="Choose what appears in the storefront sections.">
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm" data-testid="admin-homepage-placement-table">
            ...
            <tbody>
              {rows.map((product) => (
                <tr key={product.id} className="border-b border-[rgb(var(--vibe-border))] last:border-0">
                  ...
                  <td className="px-3 py-3">{categoryLabel(product.category_id ?? product.category)}</td>
...
function CategoriesAdminPanel({
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
      <Section title="Add category or filter" subtitle="Create storefront filters without touching code.">
        <div className="space-y-3">
          <AdminField label="Name">
            <input className="admin-input" value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} />
          </AdminField>
          <AdminField label="Slug">
            <input className="admin-input" value={draft.slug} onChange={(e) => setDraft({ ...draft, slug: e.target.value })} placeholder="auto from name" />
          </AdminField>
          <AdminField label="Type">
            <select className="admin-input" value={draft.type} onChange={(e) => setDraft({ ...draft, type: e.target.value })}>
          ...
          <AdminField label="Parent">
            <select className="admin-input" value={draft.parent_slug} onChange={(e) => setDraft({ ...draft, parent_slug: e.target.value })}>
          ...
          <AdminField label="Sort order">
            <input className="admin-input" value={draft.sort_order} onChange={(e) => setDraft({ ...draft, sort_order: e.target.value })} inputMode="numeric" />
          </AdminField>
          <div className="flex flex-wrap gap-2">
            <button type="button" className="admin-button" onClick={save}>Save category</button>
            <button type="button" className="admin-button admin-button-secondary" onClick={() => void onSeed()}>Seed defaults</button>
          </div>
        </div>
      </Section>
      ...
      <Section title="Categories" subtitle={`${categories.length} configured`}>
        <div className="grid gap-3 md:grid-cols-2">
          {[...CATEGORIES.map((item) => ({ ...item, type: "built-in" })), ...categories].map((category) => {
            ...
            return (
              <article key={`${key}-${label}`} className="rounded-lg border border-[rgb(var(--vibe-border))] bg-white p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
...
function productImageUrls(product: Partial<Pick<Product, "cover_image_url" | "images" | "slug" | "name">>) {
  return Array.from(
    new Set(
      [
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      notify({
        ...
        description: error instanceof Error ? error.message : "Please check the product fields and try again.",
        variant: "destructive",
      });
    } finally {
      setSaving(false);
          ...
          <div>
            ...
            <div className="grid gap-3 sm:grid-cols-[132px_1fr]">
                    ...
                    {drawerImages.slice(0, 8).map((url) => (
                      <button
                        ...
                        className={cn(
                          ...
                          activeImage === url ? "border-[#111827] ring-2 ring-[#111827]/10" : "border-border",
                        )}
                        aria-label={coverImage === url ? "Current cover image" : "Preview this product image"}
                      >
              ...
              <div className="flex-1 min-w-0">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setForm({
                      ...form,
                      cover_image_url: url,
                      images: url ? Array.from(new Set([url, ...(form.images ?? [])])) : form.images,
                    });
                    setSelectedImage(url);
                  }}
                ...
                >
                  {activeImage && coverImage === activeImage ? "Selected as cover" : "Set selected as cover"}
                </button>
                {activeImage && (
                  <a
                    href={activeImage}
