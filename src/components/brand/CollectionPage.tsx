    ...
    <div className="min-h-screen bg-ivory text-ink">
      ...
      <section className="relative bg-cream">
        <div className="relative h-[38vh] min-h-[280px] md:h-[46vh]">
          ...
          <div className="relative z-10 h-full max-w-7xl mx-auto px-4 md:px-8 flex flex-col justify-end pb-8">
            <nav aria-label="Breadcrumb" className="font-sans-ui text-xs font-medium text-ink/70">
              <Link to="/" className="hover:text-ink">Home</Link>
              <span className="mx-2">/</span>
              <span className="text-ink">{breadcrumb}</span>
            </nav>
            {eyebrow && <p className="mt-3 text-xs font-semibold uppercase tracking-[0.1em] text-ink/65">{eyebrow}</p>}
            <h1 className="mt-1 font-sans-ui text-4xl font-semibold tracking-normal md:text-6xl">{title}</h1>
            <p className="mt-2 font-sans-ui text-sm font-medium text-ink/65">{items.length} products</p>
          </div>
        </div>
      </section>
      ...
      <div className="sticky top-[57px] md:top-[73px] z-30 bg-ivory/95 backdrop-blur border-b border-ink/10">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-3 flex items-center justify-between">
          <p className="font-sans-ui text-sm font-medium text-ink/65">{items.length} results</p>
          <label className="relative font-sans-ui text-sm font-medium text-ink/70">
      ...
      <section className="mx-auto max-w-7xl px-4 md:px-8 py-8 md:py-12">
        {sorted.length === 0 ? (
          <div className="text-center py-24">
            <p className="font-sans-ui text-3xl font-semibold">Coming soon.</p>
            <p className="mt-2 text-ink/60">This collection is being restocked.</p>
            <Link to="/" className="mt-6 inline-flex items-center bg-ink text-ivory px-6 py-3 text-sm font-semibold">
