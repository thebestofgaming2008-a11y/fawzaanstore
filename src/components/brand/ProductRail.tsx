...
function defaultProductVariant(product: Product) {
  return [product.colors?.[0]?.name, product.sizes?.[0]].filter(Boolean).join(" / ");
}
    ...
    <section className="bg-ivory">
      <div className="mx-auto max-w-7xl px-5 md:px-8 pt-12 md:pt-16">
        {heading && (
          <div className="flex items-end justify-between mb-6">
            <h2 className="font-sans-ui text-2xl md:text-3xl font-semibold text-ink">{heading}</h2>
            {to && <Link to={to} className="font-sans-ui text-sm font-semibold text-ink/65 hover:text-ink">View all</Link>}
          </div>
        )}
      </div>
      <div className="overflow-x-auto scrollbar-hide snap-x snap-mandatory">
        <ul className="flex gap-3 md:gap-5 px-5 md:px-8 pb-12 md:pb-16 max-w-7xl mx-auto">
          {items.map((p) => {
            const defaultVariant = defaultProductVariant(p);
            return (
              <li key={p.slug} className="snap-start shrink-0 w-[62vw] sm:w-64 md:w-72">
              <div className="group block font-sans-ui">
                ...
                <div className="mt-3 flex items-start justify-between gap-3">
                  <p className="text-sm md:text-[15px] font-semibold leading-snug text-ink">{p.name}</p>
                  <p className="text-sm font-semibold text-ink shrink-0">{format(p.price)}</p>
                </div>
              </div>
            </li>
            );
